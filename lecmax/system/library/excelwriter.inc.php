			...
			$header = <<<EOH
				...
				<!--[if gte mso 9]><xml>
				 <o:DocumentProperties>
				  <o:LastAuthor>Hoang</o:LastAuthor>
				  <o:LastSaved>2014-04-04T07:46:23Z</o:LastSaved>
...
EOH;
		...
		*/
		...
		function writeLine($line_arr, $css = NULL)
		{
			...
			foreach($line_arr as $col)
				fwrite($this->fp,"<td class=xl24 width=120 $cssStr>$col</td>");
			fwrite($this->fp,"</tr>");
		}
		
		function writeHeader($line_arr, $numcol = 0, $css = NULL)
		{
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
			...
			fwrite($this->fp,"<tr>");
			foreach($line_arr as $col)
				fwrite($this->fp,"<td class=xl24 width=64 colspan='".$numcol."' $cssStr>$col</td>");
			fwrite($this->fp,"</tr>");
		}
		...
		*/
		function writeRow($height = NULL)
		{
			...
			if($this->newRow==false)
				fwrite($this->fp,"<tr ".$height.">");
			else
				fwrite($this->fp,"</tr><tr ".$height.">");
			$this->newRow=true;	
		}
		...
		*/
		function writeCol($value,$css = NULL)
		{
			...
			fwrite($this->fp,"<td class=xl24 width=120 $cssStr>$value</td>");
		}
		
		function writeHeaderByNumcol($line_arr, $numcol = array(), $css = NULL)
		{
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
			...
			$i=0;
			foreach($line_arr as $col){
				fwrite($this->fp,"<td class=xl24 width=64 colspan='".$numcol[$i]."' $cssStr>$col</td>");
				$i++;
			}
			fwrite($this->fp,"</tr>");
		}
