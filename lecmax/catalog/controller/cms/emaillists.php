<?php
class ControllerCmsEmaillists extends Controller {
	private $error = array();
	...
	public function __construct($registry) {
		...
		$langs = $this->model_catalog_lang->getLangByModule('emaillists',1);
	...
	public function index() {
		$this->load->model('cms/emaillists');
		...
		$this->data['emaillistss'] = $this->model_cms_emaillists->getEmaillistss($arr_path[0]);

		$template = 'emaillists.tpl';
	...
	public function detail() {
		...
		$emaillists_id = isset($this->request->get['emaillists_id'])?	(int)$this->request->get['emaillists_id']:0;

		$this->data['emaillists'] = $this->model_cms_emaillists->getEmaillists($emaillists_id);

		$template = 'emaillists_detail.tpl';
		$seo = $this->data['emaillists'];
