...
<div class="news-link center-no-paging grey">
<?php $demnew=0;foreach($item['newss'] as $child){?>
<?php
    $t1                         = $this->model_cms_news->getFriendlyUrl('news_id='.$child['news_id'],$this->config->get('config_language_id'));
    $href                       = 'HTTP_SERVER' . $this->config->get('config_language') . '/' . $item['url_current'] . '/' . $t1 .'.html';
    ...
    $image_news = empty($image_news)&&isset($image['src'])&& !empty($image['src'])?str_replace(HTTP_SERVER,'HTTP_SERVER',$image['src']):$image_news;
    $image_news = empty($image_news)?str_replace(HTTP_SERVER,'HTTP_SERVER',PATH_IMAGE_THUMB):$image_news;
?>
