<?php
class ControllerCatalogCatenews extends Controller {
	private $error = array();
	...
	public function index() {
		$this->document->setTitle($this->data['heading_title_catenews']);
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->request->post['path'] = substr('catenews',4);
			...
			$this->redirect($this->url->link('catalog/catenews', 'token=' . $this->session->data['token'], '', 'SSL'));
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $category_id) {
				//$this->model_catalog_category->updateStatus($category_id,'news');
	...
	private function getList() {
		...
		$this->data['breadcrumbs'][] = array(
		                                     'text'      => $this->data['heading_title_catenews'],
		                                     'href'      => $this->url->link('catalog/catenews', 'token=' . $this->session->data['token'], '', 'SSL'),
		                                     'separator' => ' :: '
		                                     );

		$this->data['insert'] = $this->url->link('catalog/catenews/insert', 'token=' . $this->session->data['token'], '', 'SSL');
		$this->data['delete'] = $this->url->link('catalog/catenews/delete', 'token=' . $this->session->data['token'], '', 'SSL');

		$this->data['categories'] = array();

		$results = $this->model_catalog_category->getCategories(94);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
			                  ...
			                  'href' => $this->url->link('catalog/catenews/update', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'], '')
			                  );
		...
		$this->template = 'catalog/catenews_list.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
	...
	private function getForm() {
		$this->data['parent_default'] = 94;
		
		$this->data['entry_image1'] = 'Hình banner trang chi tiết';
		$this->data['help_entry_image1'] = 'Kích thước hình 2000 x 857 px';
		...
		if (!isset($this->request->get['category_id'])) {
			$this->data['action'] = $this->url->link('catalog/catenews/insert', 'token=' . $this->session->data['token'], '', 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/catenews/update', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'], '');
		}

		$this->data['cancel'] = $this->url->link('catalog/catenews', 'token=' . $this->session->data['token'], '', 'SSL');
		...
		$categories = $this->model_catalog_category->getCategories(94);
		...
		$this->template = 'catalog/catenews_form.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
	...
	private function validateDelete() {
		...
		if(count(array_intersect(array(335, 1),$this->request->post['selected']))>0)
		{
			$this->error['warning'] = "Không thể xoá mục \"Tin tuyển dụng\". Nếu không muốn hiển thị chọn \"Sửa\" và chọn tình trạng là \"Tắt\"!";
		}
