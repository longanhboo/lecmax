<?php
class ControllerCatalogBackground extends Controller {
	private $error = array();
	...
	public function __construct($registry) {
		...
		$langs = $this->model_catalog_lang->getLangByModule('background',2);
		foreach($langs as $lang){
		...
		if(isset($this->request->get['category_id']) && $this->request->get['category_id']>0){
			...
			$this->data['heading_title'] = $this->data['heading_title'] . ' >> ' . $this->model_catalog_category->getTitle($this->request->get['category_id']);
		}
	}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/background', 'token=' . $this->session->data['token'], '', 'SSL'));
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['category_image'])) {
				foreach ($this->request->post['category_image'] as $image) {
			...
			if (isset($this->request->get['ispage'])) {
				$url .= '&ispage=' . $this->request->get['ispage'];
			}
			
			$this->session->data['success'] = $this->data['text_success_update'];

			$this->redirect($this->url->link('catalog/background/update', 'token=' . $this->session->data['token'] . $url, '', 'SSL'));
		}
	...
	private function getList() {
		...
		$results = $this->model_catalog_category->getCategories(0,2);
		//$results1 = $this->model_catalog_category->getCategoriesForProduct(94,4);

		//$results = array_merge($results2, $results1);
		
		foreach ($results as $result) {
			if($result['category_id']==94 || $result['category_id']==95 || $result['category_id']==255 || $result['category_id']==92 || $result['category_id']==96 || $result['parent_id']==98  || $result['parent_id']==97 || $result['category_id']==93 || $result['parent_id']==162 || $result['category_id']==99  || $result['category_id']==97  || $result['parent_id']==169 || $result['parent_id']==166) continue;
			$action = array();

			$action[] = array(
			                  'text' => $this->data['text_edit'],
			                  'href' => $this->url->link('catalog/background/update', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'], '')
			                  );
			...
			$this->data['categories'][] = array(
			                                    ...
			                                    'type'  		=> ($result['type']=='module')?'<a class="button goto" href="'.$href.'">Module</a>':ucfirst($result['type']),
		...
		*/
		...
		$this->template = 'catalog/background_list.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
	...
	private function getForm() {
		...
		$category_temp = $this->model_catalog_category->getCategory($category_id);
		//if($category_id==95 || $category_id==92 || $category_id==93 || $category_id==97 || (isset($category_temp['parent_id'])&& $category_temp['parent_id']==97)){
		
		
		/*if($category_id==95){
			$this->data['help_column_images'] = $this->data['help_entry_image_home'];
			$this->data['help_column_images1'] = $this->data['help_entry_image1_home'];
		}
		if($category_id==93 || $category_id==166 || $category_id==162 || $category_id==96 || $category_id==97){
			$this->data['help_column_images'] = $this->data['help_column_images1'];
			$this->data['column_images'] = $this->data['column_images1'];
		}
		
		if($category_id==169){
			$this->data['column_images1'] = $this->data['column_images'] . ' ( Đêm )';
			$this->data['help_column_images1'] = $this->data['help_column_images'];
		}*/
		
		if($category_id==95){
			$this->data['help_column_images'] = $this->data['help_column_images_home'];
		}
		...
		if($category_id==93){
			$this->data['help_column_images'] = $this->data['help_column_images_product'];
		}
		if($category_id==96){
			$this->data['help_column_images'] = $this->data['help_column_images_96'];
		}
		
		if($category_id==291){
			$this->data['help_column_images'] = $this->data['help_column_images_recruitment'];
		}
		
		if($category_id==164){
			$this->data['help_column_images'] = $this->data['help_column_images_service'];
		}
		
		if($category_id==269){
			$this->data['help_column_images'] = $this->data['help_column_images_solution'];
		}
		
		if($category_id==92){
			$this->data['help_column_images'] = $this->data['help_column_images_contact'];
		}
		
		if($category_id==295){
			$this->data['help_column_images'] = $this->data['help_column_images_project'];
			$this->data['help_column_images1'] = $this->data['help_column_images1_project'];
		}
		...
		$this->data['breadcrumbs'][] = array(
		                                     'text'      => $this->data['heading_title'],
		                                     'href'      => $this->url->link('catalog/background', 'token=' . $this->session->data['token'], '', 'SSL'),
		                                     'separator' => ' :: '
		                                     );
		...
		if (!isset($this->request->get['category_id'])) {
			$this->data['action'] = $this->url->link('catalog/background/insert', 'token=' . $this->session->data['token'] . $url, '', 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/background/update', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, '');
		}

		$this->data['cancel'] = $this->url->link('catalog/background', 'token=' . $this->session->data['token'], '', 'SSL');
		...
		$this->template = 'catalog/background_form.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
