<?php
class ControllerCatalogUngvien extends Controller {
	private $error = array();
	...
	public function __construct($registry) {
		...
		$langs = $this->model_catalog_lang->getLangByModule('ungvien',2);
	...
	public function index() {
		...
		$this->load->model('catalog/ungvien');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_catalog_ungvien->addUngvien($data);
			...
			$this->redirect($this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . $url, '', 'SSL'));
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_catalog_ungvien->editUngvien($this->request->get['ungvien_id'], $data);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $ungvien_id) {

				/*{DELETE_CONTROLLER}*/

				$this->model_catalog_ungvien->deleteUngvien($ungvien_id);
			}
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $ungvien_id) {
				$this->model_catalog_ungvien->copyUngvien($ungvien_id);
			}
	...
	private function getList() {
		...
		$this->data['breadcrumbs'][] = array(
		                                     ...
		                                     'href'      => $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . $url, '', 'SSL'),
		                                     'separator' => ' :: '
		                                     );
		...
		$this->data['insert'] = $this->url->link('catalog/ungvien/insert', 'token=' . $this->session->data['token'] . $url, '', 'SSL');
		$this->data['copy'] = $this->url->link('catalog/ungvien/copy', 'token=' . $this->session->data['token'] . $url, '', 'SSL');
		$this->data['delete'] = $this->url->link('catalog/ungvien/delete', 'token=' . $this->session->data['token'] . $url, '', 'SSL');

		$this->data['filter'] = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . $url_filter, '', 'SSL');
		$this->data['export'] = $this->url->link('catalog/ungvien/exportnhantin', 'token=' . $this->session->data['token'] . $url, '', 'SSL');
		/*{BACK}*/
		$this->data['ungviens'] = array();
		...
		/*{INCLUDE_IMAGE_TOOL}*/
		$ungvien_total = $this->model_catalog_ungvien->getTotalUngviens($data);

		$results = $this->model_catalog_ungvien->getUngviens($data);
		...
		foreach ($results as $result) {
			...
			/*$action[] = array(
			                  ...
			                  'href' => $this->url->link('catalog/ungvien/update', 'token=' . $this->session->data['token'] . '&ungvien_id=' . $result['ungvien_id'] . $url, '', 'SSL')
			                  );*/
			...
			$this->data['ungviens'][] = array(
				...
				'date_added'       => date('d-m-Y H:i', strtotime($result['date_added'])),
				  ...
				  'selected'   => isset($this->request->post['selected']) && in_array($result['ungvien_id'], $this->request->post['selected']),
				  'action'     => $action
				  );
		}
		...
		$this->data['sort_name'] = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, '', 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, '', 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, '', 'SSL');
		...
		$pagination->total = $ungvien_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->data['text_pagination'];
		$pagination->url = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . $url . '&page={page}', '', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['results'] = sprintf($this->language->get('text_pagination'), ($ungvien_total) ? (($page - 1) * $this->config->get('config_admin_limit')) + 1 : 0, ((($page - 1) * $this->config->get('config_admin_limit')) > ($ungvien_total - $this->config->get('config_admin_limit'))) ? $ungvien_total : ((($page - 1) * $this->config->get('config_admin_limit')) + $this->config->get('config_admin_limit')), $ungvien_total, ceil($ungvien_total / $this->config->get('config_admin_limit')));
		...
		$this->template = 'catalog/ungvien_list.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
	...
	private function getForm() {
		...
		//====================================================Assigne Data=======================================================
		if (!isset($this->request->get['ungvien_id'])) {
			$this->data['action'] = $this->url->link('catalog/ungvien/insert', 'token=' . $this->session->data['token'] . $url, '', 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/ungvien/update', 'token=' . $this->session->data['token'] . '&ungvien_id=' . $this->request->get['ungvien_id'] . $url, '', 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/ungvien', 'token=' . $this->session->data['token'] . $url, '', 'SSL');

		$this->data['token'] = $this->session->data['token'];

		//cate danh sach con
		 if(isset($this->request->get['cate']) && $this->request->get['cate']>0){
		 	$sublist_cate = $this->model_catalog_ungvien->getCateById($this->request->get['cate']);
		...
		if (isset($this->request->get['ungvien_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$ungvien_info = $this->model_catalog_ungvien->getUngvien($this->request->get['ungvien_id']);
		}
		...
		} elseif (isset($ungvien_info)) {
			$this->data['sort_order'] = $ungvien_info['sort_order'];
		} else {
		...
		} else if (isset($ungvien_info)) {
			$this->data['status'] = $ungvien_info['status'];
		} else {
			$this->data['status'] = 1;
		}

		//tab data
		if (isset($this->request->post['ungvien_description'])) {
			$this->data['ungvien_description'] = $this->request->post['ungvien_description'];
		} elseif (isset($ungvien_info)) {
			$this->data['ungvien_description'] = $this->model_catalog_ungvien->getUngvienDescriptions($this->request->get['ungvien_id']);
		} else {
			$this->data['ungvien_description'] = array();
		}


		$this->template = 'catalog/ungvien_form.tpl';
		$this->children = array(
		                        'common/header',
		                        'common/footer',
		                        );
	...
	private function validateForm() {
		...
		if (!$this->user->hasPermission('modify', 'catalog/ungvien')) {
			$this->error['warning'] = $this->data['error_permission'];
		}

		//chi validate ngon ngu mac dinh (khi lay du lieu kiem tra neu title khac rong moi hien thi)
		if((strlen(utf8_decode($this->request->post['ungvien_description'][$this->config->get('config_language_id')]['name'])) < 1) || (strlen(utf8_decode($this->request->post['ungvien_description'][$this->config->get('config_language_id')]['name'])) > 255))
			$this->error['name'][$this->config->get('config_language_id')] = $this->data['error_name'];

		foreach ($this->request->post['ungvien_description'] as $language_id => $value) {
			/*{VALIDATE_PDF}*/
		}
	...
	public function exportnhantin() {
		...
		if($filter_is_order==1){
			$filename="BaoCao_UngVien_".date("dmYhis").".xls";
		}else{
			$filename="BaoCao_UngVien_".date("dmYhis").".xls";
		}
		...
		$myArr=array("<img width='150' height='auto' src='".HTTP_CATALOG."/catalog/view/theme/default/images/social-share.png' />", '<br><strong>LECMAX</strong> <br>');
		if($filter_is_order==1){
			$column = array(3,4);
		}elseif($filter_is_order==0 && $filter_is_order!=null){
			$column = array(3,4);
		...
		if($filter_is_order==1){
			$myArr=array("<b style='text-transform:uppercase;'>BÁO CÁO DANH SÁCH ỨNG VIÊN</b>");
		}elseif($filter_is_order==0 || $filter_is_order==null){
			$myArr=array("<b style='text-transform:uppercase;'>BÁO CÁO DANH SÁCH ỨNG VIÊN</b>");
		}
		$excel->writeHeader($myArr, $socot, array('text-align'=>'center', 'color'=> '#030', 'font-size'=> '18px'));
		
		if($filter_phone!=null || $filter_name!=null || $filter_email!=null || $filter_date_start!=null || $filter_date_end!=null || $filter_status!=null || $filter_cmnd!=null  || $filter_company!=null || $filter_address!=null || $filter_comment!=null || $filter_is_mail!=null){
			$excel->writeLine(array(), $socot, array('text-align'=>'right', 'color'=> '#003', 'font-size'=> '14px'));
		...
		$this->load->model('catalog/ungvien');
		$results = $this->model_catalog_ungvien->getUngviens(array('sort' => 'p.sort_order', 'order' => 'ASC', 'filter_status' => $filter_status, 'filter_is_mail' => $filter_is_mail, 'filter_phone' => $filter_phone, 'filter_cmnd' => $filter_cmnd, 'filter_name' => $filter_name, 'filter_email' => $filter_email, 'filter_address' => $filter_address, 'filter_company' => $filter_company, 'filter_comment' => $filter_comment, 'filter_date_start' => $filter_date_start, 'filter_date_end' => $filter_date_end, 'filter_is_order' => $filter_is_order));
		...
		foreach ($results as $result) {
			...
			//$excel->writeCol($result['address'], array('text-align'=>'left', 'color'=> '#003', 'font-size'=> '14px', 'border'=>'#000 solid 1px'));
