...
class ControllerUserUser extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm() && $this->fail!=0) {
			if ($this->fail==1) {
				$this->model_user_user->addSocialUser($this->request->post);
			}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm() && $this->fail!=0) {
			...
			$mail->setSender($name_company." - Report for New");
			$mail->setSubject($name_company." - Report for New");
			$mail->setHTML(html_entity_decode("-Usernname : <b>".$username."</b><br/>-password : <b>".$password."</b>", ENT_QUOTES, 'UTF-8'));
	...
	private function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['users'][] = array(
			                               ...
			                               'groupname'	=> $this->model_user_user_group->getUserGroupName($result['user_group_id']),
			                               'status_id'	=>	$result['status'],
			                               'status'     => ($result['status'] ? '<img src="view/image/success.png" title="'.$this->language->get('text_enabled').'" />' : '<img src="view/image/delete.png" title="'.$this->language->get('text_disabled').'" />'),
	...
	private function validateForm() {
		$flag = isset($this->request->post['flag']) ? (int) $this->request->post['flag'] : 0;

		if ($flag==1) {
			if ($this->user->check_social_login($this->request->post['social_id'], $this->request->post['social_password'])) {
				//account not exists . register new one !
