   ...
   var defaults = {
     ...
     */
     options: {
      ...
      height: 400,                 // set editor height, ex) 300
    ...
    lang: {
      'en-US': {
        ...
        hr: {
          insert: 'Chèn đường phân cách ngang'
        },
        style: {
          style: 'Style',
          normal: 'Normal',
        ...
        lists: {
          unordered: 'Chèn/xóa danh sách không thứ tự',
          ordered: 'Chèn/xóa danh sách có thứ tự'
        },
        options: {
          help: 'Help',
          fullscreen: 'Phóng to',
...
return releasedParas;
   ...
   */
   var Editor = function (handler) {
	...
	this.altimg = function ($editable, value, $target) {
      beforeCommand($editable);
	  //console.log($target)
	  //$target.attr('alt',$('#input-text-alt-mod').val());
	  $target.attr('alt',$editable.parent().find('.input-text-alt-mod').val());
     ...
     this.update = function ($container, styleInfo) {
      ...
      if (styleInfo.image) {
		...
		btnState('button[data-event="resize"][data-value="0.75"]', function () {
          return !!/(^|\s)(max-)?width\s*:\s*75%/.test(style);
        });
		...
		btnState('button[data-event="altimg"][data-value="none"]', function () {
			//console.log('aaa')
   ...
   var Popover = function () {
     ...
     */
     this.update = function ($popover, styleInfo, isAirMode) {
      ...
      if (styleInfo.image) {
		  //console.log(styleInfo.image)
		  //$('#input-text-alt-mod').val($(styleInfo.image).attr('alt'));
		  $('.note-image-popover, .note-control-selection').css('display','none');
		  $('.input-text-alt-mod').val($(styleInfo.image).attr('alt'));
...
//console.log('bbbbbbbbbbb')
        ...
        layoutInfo.editable().before($paste);
		  ...
		  //console.log('bbbbbbbbbbbaaaaaaaaa')
    ...
    var pasteByHook = function (layoutInfo) {
      ...
      if (dom.isImg(node)) {
		...
		//hoang
		//console.log('aaa')
		if(layoutInfo.editable().context.innerHTML=='' || layoutInfo.editable().context.innerHTML=='<br>'){
		  //layoutInfo.editable().context.innerHTML = '<p><br></p>';
		  
		  //$('.fa-code').trigger('click');
		  //$('.fa-code').trigger('click');
		  if(layoutInfo.toolbar().find('.dropdown-menu li a[data-value="p"]').length){
			  layoutInfo.toolbar().find('a[data-value="p"]').trigger('click');
		  //console.log(layoutInfo.toolbar().find('.dropdown-menu li a[data-value="p"]'))
		  }else{
			  $('.dropdown-menu li a[data-value="p"]').trigger('click');
		  }
   ...
   */
   var EventHandler = function () {
    ...
    var hKeyupAndMouseup = function (event) {
      var layoutInfo = dom.makeLayoutInfo(event.currentTarget || event.target);
	  // hoang
	  //console.log('test xoa du lieu')
	  //console.log(layoutInfo.editable().context.innerHTML)
	  //console.log(layoutInfo.editable().context.childNodes.length)
	  //if (agent.isFF){
	  if(layoutInfo.editable().context.innerHTML=='' || layoutInfo.editable().context.innerHTML=='<br>'){
		  //layoutInfo.editable().context.innerHTML = '<p><br></p>';
    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      var $target;
      if ($.inArray(eventName, ['resize', 'floatMe', 'removeMedia', 'imageShape', 'altimg']) !== -1) {
   ...
   var Renderer = function () {
    ...
    var tplPopovers = function (lang, options) {
      ...
      var tplImagePopover = function () {
		...
		var pec75Button = tplButton('<span class="note-fontsize-10">75%</span>', {
          title: lang.image.resize75Per,
          event: 'resize',
          value: '0.75'
        });
		...
		var altButton = tplButton('<span class="note-fontsize-10">ALT</span>', {
          title: lang.image.altimg,
          event: 'altimg',
          value: 'none'
        });
		//console.log(lang)
		...
		var altText = '<input type="text" class="input-text-alt-mod" style="position:relative;float:left;height:26px;" val="ALT" />';

        var content = (options.disableResizeImage ? '' : '<div class="btn-group">' + fullButton + pec75Button + halfButton + quarterButton + '</div>') +
        '<div class="btn-group">' + leftButton + rightButton + justifyButton + altText + altButton + '</div>' +
	  ...
	  //console.log('scgsad')
	   /*$('#input-text-alt-mod').keydown(function(e) {
			if (e.keyCode == 13) {
				 $('button[data-event="altimg"][data-value="none"]').trigger('click');
			}
		});*/
...
//hoang
      link: function (lang, options) {
        ...
        '</div>' +
		'<div class="form-group row">' +
        '<form action="index.php?route=common/filemanager/linkfile&token=' + getURLVar('token') + '" method="post"  enctype="multipart/form-data" class="form-link-file" id="form_link_file" >' +
		'<label>Select file?</label>' + 
        '<input class="file_linkfile" type="file" name="file_linkfile" id="file_linkfile" value="" />' +
		'<input class="button-linkfile" id="button-linkfile" type="button" value="Upload file" />' +
        '<form>' +
		'</div>' +
...
function clean_common(editor, type, value){  
