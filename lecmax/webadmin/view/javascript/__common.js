...
$(document).ready(function() {
	...
	$(document).delegate('button[data-toggle=\'image\']', 'click', function(e) {
		...
		var image = $(element).parent().parent().parent().parent().find('textarea').attr('id');
		
		//hoang
		$('#dialog').remove();
		$('body').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=common/filemanager&token=' + getURLVar('token') + '&CKEditorFuncNum=1&field=' + image + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
		...
		$('#dialog').dialog({
			...
			close: function (event, ui) {
				...
				if ($('#' + image).attr('value')) {
					$.ajax({
						url: 'index.php?route=common/filemanager/imageEditor&token=' + getURLVar('token'),
						type: 'POST',
	...
	$(document).delegate('a.ui-dialog-titlebar-close', 'click', function(e) {
		e.preventDefault();
		$('#modal-image').remove();
	...
	$(document).delegate('a[data-toggle=\'image\']', 'click', function(e) {
		...
		//hoang
		$('#dialog').remove();
		$('body').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=common/filemanager&token=' + getURLVar('token') + '&field=' + encodeURIComponent($(element).parent().find('input').attr('id')) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
		$('body').css({'overflow':'hidden'});
		
		var Yheight = $(window).height();
		var preview = $(element).attr('id');

		$('#dialog').dialog({
			title: 'Quản lý hình ảnh',
			close: function (event, ui) {
				$('body').css({'overflow':'auto'});
				if ($('#' + $(element).parent().find('input').attr('id')).attr('value')) {
					$.ajax({
						url: 'index.php?route=common/filemanager/image&token=' + getURLVar('token'),
						type: 'POST',
						data: 'image=' + encodeURIComponent($('#' + $(element).parent().find('input').attr('id')).attr('value')),
						dataType: 'text',
						success: function(data) {
							$('#' + preview).html('<img src="' + data + '" alt="" title="" data-placeholder="" />');
						}
					});
				}
			},	
	...
	$(document).ajaxStop(function() {
		//hoang $('[data-toggle=\'tooltip\']').tooltip({container: 'body'});
	});
