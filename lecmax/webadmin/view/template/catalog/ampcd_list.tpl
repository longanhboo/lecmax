					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<?php if ($ampcds) { ?>
								<?php foreach ($ampcds as $ampcd) { ?>
								<tr>
									<td class="text-center" style="display:none"><?php if ($ampcd['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $ampcd['ampcd_id']; ?>" checked="checked" />
										<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $ampcd['ampcd_id']; ?>" />
										<?php } ?>
									</td>
									<td class="text-center" style="display:none" ><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" ampcdid="<?php echo $ampcd['ampcd_id'];?>" value="<?php echo $ampcd['sort_order'];?>" /></td>
									<td class="text-center" style="display:none"><?php if(!empty($ampcd['image'])){?><img src="<?php echo $ampcd['image']; ?>" alt="<?php echo $ampcd['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $ampcd['name']; ?> - <?php echo $ampcd['name1']; ?></td>
                                    <!--{VALUE}-->
									<td class="text-center">
                                    	<?php //echo $ampcd['status'];?>
										<?php if ($ampcd['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" ampcdid="<?php echo $ampcd['ampcd_id'];?>" data-status="<?php echo $ampcd['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" ampcdid="<?php echo $ampcd['ampcd_id'];?>" data-status="<?php echo $ampcd['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($ampcd['action'] as $action) { ?>
		...
		$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=ampcd&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=ampcd&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=ampcd&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=ampcd&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
