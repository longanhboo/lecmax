					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									<td class="text-center"><?php if ($city['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" cityid="<?php echo $city['city_id'];?>" value="<?php echo $city['sort_order'];?>" /></td>
									<!--{VALUE_IMAGE}-->
									<td class="text-left"><?php echo $city['name']; ?></td>
                                    <!--{VALUE}-->
									<td class="text-center">
										<?php if ($city['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" cityid="<?php echo $city['city_id'];?>" data-status="<?php echo $city['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" cityid="<?php echo $city['city_id'];?>" data-status="<?php echo $city['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($city['action'] as $action) { ?>
		...
		$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=city&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=city&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=city&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=city&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
