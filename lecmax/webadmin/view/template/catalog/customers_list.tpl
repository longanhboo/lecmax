					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									<td class="text-center"><?php if ($customers['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" customersid="<?php echo $customers['customers_id'];?>" value="<?php echo $customers['sort_order'];?>" /></td>
									<td class="text-center"><?php if(!empty($customers['image'])){?><img src="<?php echo $customers['image']; ?>" alt="<?php echo $customers['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $customers['name']; ?></td>
                                    <!--{VALUE}-->
                                    <td class="text-center status-en-display"><?php echo $customers['status_en']; ?></td>
									<td class="text-center">
										<?php if ($customers['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" customersid="<?php echo $customers['customers_id'];?>" data-status="<?php echo $customers['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" customersid="<?php echo $customers['customers_id'];?>" data-status="<?php echo $customers['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($customers['action'] as $action) { ?>
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				...
				$.ajax({
					...
					type: 'POST',
					data: 't=customers&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=customers&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=customers&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=customers&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
