...
<div id="content">
				...
				<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									<td <?php if($superadmin==false) echo 'style="display:none"';?> class="text-center" width="1"><input type="checkbox" onclick="$('input[name*=\'selected\']').attr('checked', this.checked);" /></td>
									<td <?php if($superadmin==false) echo 'style="display:none"';?> class="text-center" width="70px">
							...
							<tbody>
								<tr <?php if($superadmin==false) echo 'style="display:none"';?> class="filter">
									<td></td>
									<td></td>
									<!--{FILTER_IMAGE}-->
									...
									<td class="text-center">
										<select id="filter_status" name="filter_status" class="form-control" style="min-width:100px">
											<option value="*"></option>
								...
								<?php if ($templatess) { ?>
								<?php foreach ($templatess as $templates) { ?>
								<tr>
									<td <?php if($superadmin==false) echo 'style="display:none"';?> class="center"><?php if ($templates['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $templates['templates_id']; ?>" checked="checked" />
										<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $templates['templates_id']; ?>" />
										<?php } ?>
									</td>
									<td <?php if($superadmin==false) echo 'style="display:none"';?> class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" templatesid="<?php echo $templates['templates_id'];?>" value="<?php echo $templates['sort_order'];?>" /></td>
									<!--{VALUE_IMAGE}-->
									<td class="text-left"><?php echo $templates['name']; ?></td>
									<!--{VALUE}-->
									<td class="text-center">
										<?php if ($templates['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" templatesid="<?php echo $templates['templates_id'];?>" data-status="<?php echo $templates['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" templatesid="<?php echo $templates['templates_id'];?>" data-status="<?php echo $templates['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($templates['action'] as $action) { ?>
	...
	<script type="text/javascript">
		function filter() {
			url = 'index.php?route=catalog/templates&token=<?php echo $token; ?>';
			...
			$(".sort_order").blur(function(){
				...
				if(isNaN(sort_order)===false)
				{
					$.ajax({
						...
						type: 'POST',
						data: 't=templates&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
						dataType: 'text',
