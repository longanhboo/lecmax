	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<a <?php if(isset($cate_service['typeservice']) && $cate_service['typeservice']!='2') echo 'style="display:none"';?> onclick="location = '<?php echo $insert; ?>'" data-toggle="tooltip" title="<?php echo $button_insert; ?>" class="btn btn-primary"><i class="fa fa-plus"></i> <lbl class="s-mobile"><?php echo $button_insert; ?></lbl></a>
				...
				<button <?php if(isset($cate_service['typeservice']) && $cate_service['typeservice']!='2') echo 'style="display:none"';?> type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>" class="btn btn-danger" onclick="$('#form').submit();"><i class="fa fa-trash-o"></i> <lbl class="s-mobile"><?php echo $button_delete; ?></lbl></button>
                	<?php if($cate>0){?>
	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
				...
				<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									<td class="text-center"  width="1" <?php if(isset($cate_service['typeservice']) && $cate_service['typeservice']!='2') echo 'style="display:none"';?> ><input type="checkbox" onclick="$('input[name*=\'selected\']').attr('checked', this.checked);" /></td>
            ...
            <!--{COLUMN}-->
							...
							<tbody>
								<tr>
									<td <?php if(isset($cate_service['typeservice']) && $cate_service['typeservice']!='2') echo 'style="display:none"';?> ></td>
									<td></td>
            ...
            <!--{FILTER}-->
								...
								<tr>
									<td class="text-center" <?php if(isset($cate_service['typeservice']) && $cate_service['typeservice']!='2') echo 'style="display:none"';?> ><?php if ($service['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" serviceid="<?php echo $service['service_id'];?>" value="<?php echo $service['sort_order'];?>" /></td>
									<td class="text-center" style="display:none"><?php if(!empty($service['image'])){?><img src="<?php echo $service['image']; ?>" alt="<?php echo $service['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $service['name']; ?></td>
                                    			<td class="text-center" style="display:none"><?php echo $service['ishome']; ?></td>
            
            <!--{VALUE}-->
                                    <td class="text-center status-en-display"><?php echo $service['status_en']; ?></td>
									<td class="text-center">
										<?php if ($service['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" serviceid="<?php echo $service['service_id'];?>" data-status="<?php echo $service['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" serviceid="<?php echo $service['service_id'];?>" data-status="<?php echo $service['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($service['action'] as $action) { ?>
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				...
				$.ajax({
					...
					type: 'POST',
					data: 't=service&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=service&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=service&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=service&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
