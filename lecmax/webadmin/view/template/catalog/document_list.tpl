	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
            ...
            <!--{FILTER}-->
								...
								<tr>
									<td class="text-center"><?php if ($document['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" documentid="<?php echo $document['document_id'];?>" value="<?php echo $document['sort_order'];?>" /></td>
									<td class="text-center"><?php if(!empty($document['image'])){?><img src="<?php echo $document['image']; ?>" alt="<?php echo $document['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $document['name']; ?></td>
                                    			<td class="text-center" style="display:none"><?php echo $document['ishome']; ?></td>
            
            <!--{VALUE}-->
                                    <td class="text-center status-en-display"><?php echo $document['status_en']; ?></td>
									<td class="text-center">
										<?php if ($document['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" documentid="<?php echo $document['document_id'];?>" data-status="<?php echo $document['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" documentid="<?php echo $document['document_id'];?>" data-status="<?php echo $document['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($document['action'] as $action) { ?>
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				...
				$.ajax({
					...
					type: 'POST',
					data: 't=document&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=document&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=document&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=document&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
