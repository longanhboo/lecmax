	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right"  >
                ...
                <a style="display:none" onclick="location = document.URL" data-toggle="tooltip" title="<?php echo $button_refresh; ?>" class="btn btn-primary"><i class="fa fa-refresh"></i> <lbl class="s-mobile"><?php echo $button_refresh; ?></lbl></a>
                	<?php if($cate>0){?>
					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									<td class="text-center" ><?php if ($contact['selected']) { ?>
									...
									<td class="text-center"  ><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" contactid="<?php echo $contact['contact_id'];?>" value="<?php echo $contact['sort_order'];?>" /></td>
									<td class="text-center"  style="display:none" ><?php if(!empty($contact['image'])){?><img src="<?php echo $contact['image']; ?>" alt="<?php echo $contact['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $contact['name']; ?></td>
                                    <td class="text-center"  ><?php echo $contact['ishome']; ?></td>
                                    <!--{VALUE}-->
									<td class="text-center">
										<?php if ($contact['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" contactid="<?php echo $contact['contact_id'];?>" data-status="<?php echo $contact['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" contactid="<?php echo $contact['contact_id'];?>" data-status="<?php echo $contact['status_id'] ?>">
										<?php endif ?>
									</td>
                      ...
                      <table  class="table table-bordered table-hover">
                        <tr>
                          ...
                          <label class="col-sm-2 control-label"  style="width:100% !important;">
                            <span data-toggle="tooltip" title="<?php echo $help_entry_image_contact; ?>"><?php echo $entry_image_contact; ?></span><br/>
                            <p class="s-mobile s-mobile-help"><?php echo $help_entry_image_contact ?></p>
                          </label>
                          </td>
                          <td>
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=contact&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&cate=' + <?php echo $cate; ?> + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=contact&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&cate=' + <?php echo $cate; ?>,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=contact&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=contact&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
