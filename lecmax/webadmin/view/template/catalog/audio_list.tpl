...
<div id="content">
	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
            ...
            <!--{FILTER}-->
								...
								<tr>
									<td class="text-center"><?php if ($audio['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $audio['audio_id']; ?>" checked="checked" />
										<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $audio['audio_id']; ?>" />
										<?php } ?>
									</td>
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" audioid="<?php echo $audio['audio_id'];?>" value="<?php echo $audio['sort_order'];?>" /></td>
									<td class="text-center" style="display:none"><?php if(!empty($audio['image'])){?><img src="<?php echo $audio['image']; ?>" alt="<?php echo $audio['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $audio['name']; ?></td>
                                    			<td <?php if($sublist_cate>0) echo 'style="display:none"';?> class="text-left"><?php echo $audio['category']; ?></td>

            			<td class="text-center"><?php echo $audio['ishome']; ?></td>
            
            <!--{VALUE}-->
									<td class="text-center">
										<?php if ($audio['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" audioid="<?php echo $audio['audio_id'];?>" data-status="<?php echo $audio['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" audioid="<?php echo $audio['audio_id'];?>" data-status="<?php echo $audio['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($audio['action'] as $action) { ?>
	...
	<script type="text/javascript">
		function filter() {
			url = 'index.php?route=catalog/audio&token=<?php echo $token; ?>';//'<?php echo $filter;?>';
		...
		$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=audio&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=audio&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=audio&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=audio&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
