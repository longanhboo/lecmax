...
<div id="content">
	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
            ...
            <!--{FILTER}-->
								...
								<tr>
									<td class="text-center" ><?php if ($video['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" videoid="<?php echo $video['video_id'];?>" value="<?php echo $video['sort_order'];?>" /></td>
									<td class="text-center" ><?php if(!empty($video['image'])){?><img src="<?php echo $video['image']; ?>" alt="<?php echo $video['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $video['name']; ?></td>
                                    <td class="text-left" style="display:none" ><?php echo $video['category']; ?></td>
                                    			<td class="text-center"  style="display:none" ><?php echo $video['ishome']; ?></td>
            
            <!--{VALUE}-->
									<td class="text-center">
										<?php if ($video['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" videoid="<?php echo $video['video_id'];?>" data-status="<?php echo $video['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" videoid="<?php echo $video['video_id'];?>" data-status="<?php echo $video['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($video['action'] as $action) { ?>
                      ...
                      <table  class="table table-bordered table-hover">
                        <tr>
                          ...
                          <label class="col-sm-2 control-label"  style="width:100% !important;">
                            <span data-toggle="tooltip" title="<?php echo $help_entry_image_video_bg; ?>"><?php echo $entry_image_video_bg; ?></span><br/>
                            <p class="s-mobile s-mobile-help"><?php echo $help_entry_image_video_bg ?></p>
                          </label>
                          </td>
                          <td>
	...
	<script type="text/javascript">
		function filter() {
			url = 'index.php?route=catalog/video&token=<?php echo $token; ?>';//'<?php echo $filter;?>';
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=video&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=video&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=video&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=video&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
