...
<div id="content">
			...
			<div class="panel-body">
              ...
              <select class="form-control" style="min-width:100px" id="filter_category" name="filter_category">
                  ...
                  <?php foreach($categories as $item){?>
                  	<option value="<?php echo $item['city_id'];?>" <?php if($filter_category==$item['city_id']) echo ' selected="selected"';?>><?php echo $item['name'];?></option>
                  <?php }?>
								...
								<tr>
									<td class="text-center"><?php if ($district['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" districtid="<?php echo $district['district_id'];?>" value="<?php echo $district['sort_order'];?>" /></td>
									<!--{VALUE_IMAGE}-->
									<td class="text-left"><?php echo $district['name']; ?></td>
                                    <td class="text-left"><?php echo $district['category']; ?></td>
                                    <!--{VALUE}-->
									<td class="text-center">
										<?php if ($district['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" districtid="<?php echo $district['district_id'];?>" data-status="<?php echo $district['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" districtid="<?php echo $district['district_id'];?>" data-status="<?php echo $district['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($district['action'] as $action) { ?>
	...
	<script type="text/javascript">
		function filter() {
			url = 'index.php?route=catalog/district&token=<?php echo $token; ?>';//'<?php echo $filter;?>';
		...
		$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=district&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=district&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=district&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=district&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
