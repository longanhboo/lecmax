	...
	<div class="container-fluid">
		...
		<div class="panel panel-default">
						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<td class="text-center" <?php if(!$cate) echo 'style="display:none"';?> ><?php echo $column_image; ?></td>
									<td class="text-left"><?php if ($sort == 'pd.name') { ?>
										...
										<?php } ?>
												...
												<td class="text-center" <?php if(!$cate) echo 'style="display:none"';?> ><?php echo $column_ishome;?></td> 
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="filter_name" name="filter_name" value="<?php echo $filter_name; ?>"  class="form-control" /></td>
                                    			<td  <?php if(!$cate) echo 'style="display:none"';?> ><select id="filter_ishome" name="filter_ishome" class="form-control" style="min-width:100px">
                  <option value="*"></option>
            ...
            <!--{FILTER}-->
								...
								<tr>
									<td class="text-center"><?php if ($project['selected']) { ?>
									...
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" projectid="<?php echo $project['project_id'];?>" value="<?php echo $project['sort_order'];?>" /></td>
									<td class="text-center" <?php if(!$cate) echo 'style="display:none"';?> ><?php if(!empty($project['image'])){?><img src="<?php echo $project['image']; ?>" alt="<?php echo $project['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $project['name']; ?></td>
                                    			<td class="text-center"  <?php if(!$cate) echo 'style="display:none"';?> ><?php echo $project['ishome']; ?></td>
            
            <!--{VALUE}-->
                                    <td class="text-center status-en-display"><?php echo $project['status_en']; ?></td>
									<td class="text-center">
										<?php if ($project['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" projectid="<?php echo $project['project_id'];?>" data-status="<?php echo $project['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" projectid="<?php echo $project['project_id'];?>" data-status="<?php echo $project['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($project['action'] as $action) { ?>
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				...
				$.ajax({
					...
					type: 'POST',
					data: 't=project&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order  + '&cate=' + <?php echo $cate; ?> + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=project&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&cate=' + <?php echo $cate; ?> ,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=project&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=project&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
