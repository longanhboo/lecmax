...
<div id="content">
	...
	<div class="container-fluid">
    	<!--<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Không được xoá mục "Hỏi & Đáp" và "Video". Nếu không muốn hiển thị chọn "Sửa" và chọn tình trạng là "Tắt"!
		...
		/*$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				...
				$.ajax({
					...
					type: 'POST',
					data: 't=category&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&cate_type=catenews' + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=category&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&cate_type=catenews',
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=category&id=' + encodeURIComponent(id) + '&status=' + status + '&cate_type=catenews',
				dataType: 'text',
				success: function(data) {
