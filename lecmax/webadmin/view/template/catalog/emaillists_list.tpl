					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<?php if ($emaillistss) { ?>
								<?php foreach ($emaillistss as $emaillists) { ?>
								<tr>
									<td class="text-center"><?php if ($emaillists['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $emaillists['emaillists_id']; ?>" checked="checked" />
										<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $emaillists['emaillists_id']; ?>" />
										<?php } ?>
									</td>
									<td class="text-center"><input type="text" onkeypress="return checkIt(event);" style="width:25px" class="sort_order text-center" emaillistsid="<?php echo $emaillists['emaillists_id'];?>" value="<?php echo $emaillists['sort_order'];?>" /></td>
									<td class="text-center" style="display:none"><?php if(!empty($emaillists['image'])){?><img src="<?php echo $emaillists['image']; ?>" alt="<?php echo $emaillists['name']; ?>" style="padding: 1px; border: 1px solid #DDDDDD;" /><?php }?></td>
									<td class="text-left"><?php echo $emaillists['name']; ?></td>
                                    <!--{VALUE}-->
									<td class="text-center">
										<?php if ($emaillists['status_id']==1): ?>
											<input type="checkbox" checked="checked" class="status" emaillistsid="<?php echo $emaillists['emaillists_id'];?>" data-status="<?php echo $emaillists['status_id'] ?>">
										<?php else: ?>
											<input type="checkbox" class="status" emaillistsid="<?php echo $emaillists['emaillists_id'];?>" data-status="<?php echo $emaillists['status_id'] ?>">
										<?php endif ?>
									</td>
									<td class="text-right"><?php foreach ($emaillists['action'] as $action) { ?>
		...
		$(".sort_order").click(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					...
					type: 'POST',
					data: 't=emaillists&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order + '&isclick=1',
					dataType: 'text',
		...
		$(".sort_order").blur(function(){
			...
			if(isNaN(sort_order)===false)
			{
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=emaillists&id=' + encodeURIComponent(id) + '&sort_order=' + sort_order,
					dataType: 'text',
					success: function(data) {
		...
		$(".ishome").on('click', function(){
			...
			if(isNaN(ishome)===false) {
				$.ajax({
					url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
					type: 'POST',
					data: 't=emaillists&id=' + encodeURIComponent(id) + '&ishome=' + ishome,
					dataType: 'text',
					success: function(data) {
	...
	$(".status").on('click', function(){
		...
		if(isNaN(status)===false) {
			$.ajax({
				url: 'index.php?route=catalog/sortorder&token=<?php echo $token; ?>',
				type: 'POST',
				data: 't=emaillists&id=' + encodeURIComponent(id) + '&status=' + status,
				dataType: 'text',
				success: function(data) {
